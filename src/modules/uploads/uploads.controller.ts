...
export const uploadImage: RequestHandler = async (req, res) => {
  const result = await uploadsService.uploadImage(req.user?.id, req.userRole, req.body);
  success(res, result, "Image uploaded successfully", 201);
};

export const uploadImages: RequestHandler = async (req, res) => {
  const result = await uploadsService.uploadImages(req.user?.id, req.userRole, req.body);
  success(res, result, "Images uploaded successfully", 201);
};

export const uploadFile: RequestHandler = async (req, res) => {
  const result = await uploadsService.uploadFile(req.user?.id, req.userRole, req.body);
